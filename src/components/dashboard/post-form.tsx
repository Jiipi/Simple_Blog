...
export function PostForm({ post }: PostFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!isEditing) {
        postData.slug = `${baseSlug}-${Math.random().toString(36).substring(2, 8)}`
      }
        ...
        <div className="flex justify-between items-end mb-1">
          ...
          <div>
            ...
            <label 
              ...
              className={`cursor-pointer inline-block text-sm px-3 py-1 rounded border ${uploadingImage ? 'bg-gray-100 text-gray-400' : 'bg-white text-blue-600 border-blue-200 hover:bg-blue-50'} transition-colors`}
            >
              {uploadingImage ? 'Đang tải lên...' : '+ Chèn ảnh'}
            </label>
          </div>
        </div>
        <textarea
